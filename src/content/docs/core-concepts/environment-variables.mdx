...
:::note[`HOST` and `PORT` are special]
They are only consulted by `envgo run` / `envgo run dev`, and only when the
corresponding flag is still at its default. See
...
mismatches in either direction — it never compares values.
